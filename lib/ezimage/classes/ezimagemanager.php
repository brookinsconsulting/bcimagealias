...
class eZImageManager
{
    ...
    */
    function readImageAliasesFromOriginalINI( $iniFile = false )
    {
        if ( !$iniFile )
            $iniFile = 'image.ini';
        $ini = eZINI::instance( $iniFile, 'settings', null, null, false, true );
        if ( !$ini )
            return false;
    ...
    */
    ...
    /*!
     ...
     \return the Image Alias structure.
    */
    function createAliasFromOverrideINI( $siteAccess = false, $iniFile = false, $iniGroup )
    {
        $ini = eZINI::instance( $iniFile, 'settings/siteaccess/' . $siteAccess, null, null, null, true );
        if ( !$ini->hasGroup( $iniGroup ) )
        {
